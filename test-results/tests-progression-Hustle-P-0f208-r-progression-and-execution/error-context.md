...
- Name: tests/progression.spec.ts >> Hustle Progression Verification >> Verify r_labor progression and execution
- Location: tests/progression.spec.ts:39:5
...
Call log:
  ...
  - waiting for locator('button:has-text("CONFIRM UPGRADE")')
    34 × locator resolved to <button disabled class="w-full py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-[10px] font-black rounded uppercase tracking-widest transition-all">CONFIRM UPGRADE ⚡</button>
...
```ts
  ...
  2   |
  3   | const HUSTLES = [
  4   |   'r_labor', 'r_delivery', 'tech_flip', 'cc', 'pod', 'audio', 'drop', 'vintage',
  5   |   'saas_mvp', 'agency_scale', 'ecom_brand', 'festival', 'global_franchise',
  6   |   'venture_capital', 'real_estate_empire', 'policy_flip', 'global_index'
  7   | ];
  8   |
  9   | test.describe('Hustle Progression Verification', () => {
  ...
  16  |     await page.fill('input', 'VERIFY');
  17  |     await page.press('input', 'Enter');
  18  |
